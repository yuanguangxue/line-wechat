...
import com.hp.repository.DeviceRepository;
import com.hp.repository.PushMsgRepository;
import com.hp.util.QueryStringParser;
...
public class ConnectionHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        if (userDevice != null && "1".equals(userDevice.getStatus())) {
            ...
            receivedOfflineMsgList(deviceId);
    ...
    private void receivedOfflineMsgList(String deviceId) {
        Sort sort = new Sort(new Sort.Order(Sort.Direction.ASC,"createdAt"));
        List<PushMsg> offlineMsgList = pushMsgRepository.findByAudienceDeviceId(deviceId, sort);
        for (PushMsg pushMsg : offlineMsgList) {
